...
Shared CI/CD building blocks for the Exaptation repos. Reusable GitHub Actions
live in their own subdirectory and are **versioned independently** via
release-please.
...
- `@build-harbor-image-v1` — sliding major; gets every backward-compatible fix (recommended).
- `@build-harbor-image-v1.2` — sliding minor.
- `@build-harbor-image-v1.2.3` — immutable exact release.
...
1. cuts the immutable tag + GitHub Release, e.g. `build-harbor-image-v1.2.3`
   (`include-component-in-tag`), and
2. moves the sliding `build-harbor-image-v1` / `-v1.2` tags to that commit (the
