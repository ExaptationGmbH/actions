...
That single step logs into `harbor.example.com`, builds `./Dockerfile`
for `linux/amd64` with GHA layer caching, and pushes
`harbor.example.com/myproject/myimage:1.2.3` and `:latest`.

## Inputs

| Input | Required | Default | Notes |
|-------|----------|---------|-------|
| `registry` | **yes** | — | Harbor hostname, e.g. `harbor.example.com`. |
...
- uses: docker/metadata-action@v5
  id: meta
  with:
    images: harbor.example.com/myproject/myimage
    tags: |
      type=sha,prefix=
